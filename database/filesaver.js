...
module.exports={
    ...
    getFile:(query)=>{
        return new Promise(async(resolve,reject)=>{
            await db.get().collection(collection.FILE_COLLECTION).find({mediaId:query}).then((res)=>{
                resolve(res)
            })
        })
    },
    ...
    getFile2:(query2)=>{
        return new Promise(async(resolve,reject)=>{
            await db.get().collection(collection.FILE_COLLECTION).findOne({UniqueId:query2}).toArray().then((res2)=>{
                resolve(res2)
            })
        })
    },
